...
# Usage: bundle exec rails coursemology:seed_600_gradebook
# Creates a demo course with 3 categories, multiple assessments, and 600 students
# with varied grades for demonstrating the gradebook.

namespace :coursemology do
  task seed_600_gradebook: 'db:seed' do
    ...
    ActsAsTenant.with_tenant(Instance.default) do
      ...
      course = Course.create!(
        title: 'Gradebook 600 Demo Course',
        published: true,
        enrollable: false,
        creator: admin,
        updater: admin
      ...
      # 600 users: student1000 .. student1599
      student_profiles = (1000..1599).map do |n|
        name = "student#{n}"

        # Deterministic but mixed distribution across tiers.
        tier =
          case n % 10
          when 0, 1, 2 then :high
          when 3, 4, 5, 6 then :mid
          when 7, 8 then :low
          else :sparse
          end
