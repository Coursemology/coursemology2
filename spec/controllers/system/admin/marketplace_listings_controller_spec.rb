  ...
  with_tenant(:instance) do
    ...
    describe 'GET #index' do
      ...
      it 'lists a published listing with its published vintage, provenance and adoption count' do
        ...
        # Raw timestamps, not a pre-formatted "Jan 2026 – May 2026": the client formats the range.
        expect(Time.zone.parse(row['sourceStartedAt'])).to be_within(1.second).of(course.start_at)
        expect(Time.zone.parse(row['sourceEndedAt'])).to be_within(1.second).of(course.end_at)
      ...
      # request reached Rails on whenever a proxy sits in front — the whole development setup, where
      # the browser is on the dev server's port and Rails on its own. A controller's `url_options`
      # always supplies `port: request.optional_port`, and Rails reads a port out of the `host:`
      # option only when no `:port` key is present, so the request's port silently won and every
      # link pointed at a port the browser cannot reach. Reproduced by moving the request off the
      # instance's port, which is what the proxy does.
      ...
      it 'marks a listing whose authoring copy lives in the container as marketplace-hosted' do
        ...
        listing = Course::Assessment::Marketplace::PublishService.
                  publish(create(:assessment, course: container), admin)
    ...
    describe 'GET #show' do
      ...
      it 'reports a container-hosted authoring copy, matching the index' do
        listing.update!(authoring_assessment: create(:assessment, course: create(:course, preview: true)))
