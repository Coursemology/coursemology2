  ...
  with_tenant(:instance) do
    ...
    describe '#aggregate_errors' do
      ...
      context 'when the CSV has an external ID already taken by an existing course user' do
        ...
        it 'surfaces a duplicate external ID error message' do
          subject
          errors = controller.send(:aggregate_errors)
          expect(errors).not_to be_empty
          expect(errors.any? { |e| e.include?('EXT_DUPE') }).to be(true)
        end

        it 'does not report the error as an enrolled or pending invitation error' do
          subject
          errors = controller.send(:aggregate_errors)
          expect(errors.none? { |e| e.include?('already enrolled') || e.include?('pending invitation') }).
            to be(true)
        end
      end
      ...
      context 'when a form invite has a duplicate external ID' do
        ...
        it 'includes the duplicate external ID in the error' do
          subject
          expect(response.parsed_body['errors'].any? { |e| e.include?('FORM_DUPE') }).to be(true)
        end
      end
    end
