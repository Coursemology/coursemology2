  ...
  with_tenant(:instance) do
    ...
    let(:base_params) do
      { course_id: course.id, format: :json,
        mappings: mappings, identifierMode: 'external_id', identifierColumn: 'External ID',
        csvData: csv_data }
    end
      ...
      context 'as a manager' do
        ...
        it 'returns conflicts when a grade already exists' do
          ...
          service = Course::Gradebook::ExternalAssessmentImportService.new(
            ...
            identifier_mode: 'external_id', identifier_column: 'External ID',
        ...
        it 'returns 422 on an unsupported identifier mode' do
          post :preview, params: base_params.merge(identifierMode: 'student_id')
          expect(response).to have_http_status(:unprocessable_entity)
          expect(JSON.parse(response.body)['errors']['message']).to eq('bad_identifier_mode')
        end
    ...
    describe '#create (commit)' do
      ...
      context 'as a manager' do
        ...
        it 'zeroes the weight of created components when the weighted view is disabled' do
          # base_params sends weight: 30, but the course is not weighting grades.
          post :create, params: base_params.merge(onConflict: 'replace')
          external = Course::ExternalAssessment.for_course(course).find_by(title: 'Midterm')
          expect(external.gradebook_contribution.weight).to eq(0)
        end

        it 'keeps the weight of created components when the weighted view is enabled' do
          enable_weighted_view!
          post :create, params: base_params.merge(onConflict: 'replace')
          external = Course::ExternalAssessment.for_course(course).find_by(title: 'Midterm')
