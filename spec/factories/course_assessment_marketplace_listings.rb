...
FactoryBot.define do
  factory :course_assessment_marketplace_listing,
    ...
    # Mirrors the post-Slice-2 shape: a listing whose served content is a snapshot distinct from the
    # authoring copy. The stand-in snapshot is created in the origin's own course rather than the
    # shared preview container, so unrelated specs neither pay for container (and preview-instance)
    # creation nor grow it. The real container path is covered by `publish_service_spec.rb`.
    trait :versioned do
