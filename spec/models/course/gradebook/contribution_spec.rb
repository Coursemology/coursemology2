...
RSpec.describe Course::Gradebook::Contribution do
  ...
  with_tenant(:instance) do
    ...
    describe 'validations' do
      it 'is valid with a tab and matching course' do
        expect(build(:course_gradebook_contribution, tab: tab, course: course)).to be_valid
      end

      it 'requires a tab' do
        contribution = build(:course_gradebook_contribution, tab: tab, course: course)
        contribution.tab = nil
        expect(contribution).not_to be_valid
      end

      it 'enforces one row per tab' do
        create(:course_gradebook_contribution, tab: tab, course: course)
        duplicate = build(:course_gradebook_contribution, tab: tab, course: course)
        expect(duplicate).not_to be_valid
      end

      it 'rejects a course that does not match the tab course' do
        contribution = build(:course_gradebook_contribution, tab: tab)
        contribution.course = create(:course)
        expect(contribution).not_to be_valid
      end

      it 'rejects a negative weight' do
        contribution = build(:course_gradebook_contribution, tab: tab, course: course, weight: -1)
        expect(contribution).not_to be_valid
      end

      it 'rejects a weight above 100' do
        contribution = build(:course_gradebook_contribution, tab: tab, course: course, weight: 101)
        expect(contribution).not_to be_valid
      end

      it 'accepts a weight of exactly 100' do
        contribution = build(:course_gradebook_contribution, tab: tab, course: course, weight: 100)
      ...
      it 'rejects a negative keep_highest' do
        contribution = build(:course_gradebook_contribution, tab: tab, course: course, keep_highest: -1)
        expect(contribution).not_to be_valid
      end

      it 'rejects a non-integer keep_highest' do
        contribution = build(:course_gradebook_contribution, tab: tab, course: course, keep_highest: 1.5)
        expect(contribution).not_to be_valid
      end

      it 'defaults weight 0, mode equal, keep_highest 0' do
        contribution = create(:course_gradebook_contribution, tab: tab, course: course)
        expect(contribution.weight).to eq(0)
        expect(contribution.weight_mode).to eq('equal')
        expect(contribution.keep_highest).to eq(0)
      end
    end
    ...
    describe '.bulk_update' do
      ...
      it 'is transactional — an invalid value rolls everything back' do
        ...
        create(:course_gradebook_contribution, tab: tab2, course: course, weight: 20)
        expect do
          described_class.bulk_update(
            course: course,
      ...
      it 'persists keep_highest from the update entry' do
        described_class.bulk_update(
          course: course,
          updates: [tab_id: tab1.id, weight: 60, keep_highest: 2]
        )
        expect(described_class.find_by(tab_id: tab1.id).keep_highest).to eq(2)
      end

      it 'defaults keep_highest to 0 when the entry omits it' do
        described_class.bulk_update(
          course: course,
          updates: [tab_id: tab1.id, weight: 60]
        )
