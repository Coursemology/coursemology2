  ...
  with_tenant(:instance) do
    ...
    describe '#can_read_grade?' do
      ...
      before do
        ...
        # `ability.can?(:grade, submission)` — `submission` resolves to an Attempt, and CanCan's
        # `can :grade, Course::Assessment::Answer, submission: { assessment: ... }` rule matches on
        # the Answer subject, not its Attempt.
