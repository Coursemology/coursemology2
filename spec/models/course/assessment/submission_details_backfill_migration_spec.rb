...
RSpec.describe CreateCourseAssessmentSubmissionDetails, type: :model do
  ...
  # rather than the submission factory for two reasons: the factory also creates the
  # course_assessment_submission_details row that this backfill is meant to create, so it cannot
  # seed a base row lacking one; and raw SQL lets each course-coupled column be given an exact,
  # asserted value. Every base NOT NULL column is set; the columns the migration copies
  ...
  with_tenant(:instance) do
    ...
    it 'is idempotent — re-running the backfill adds no duplicate detail row' do
      base_id = insert_base_submission(assessment: assessment, creator: creator, session_id: 'idempotent')

      described_class.new.up
      described_class.new.up
      @migration_reapplied = true

      expect(detail_count(base_id)).to eq(1)
    end

    it 'backfills only base rows still missing a detail row, leaving existing ones untouched' do
      first_id = insert_base_submission(assessment: assessment, creator: creator, session_id: 'first')
      described_class.new.up

      newer_creator = create(:course_student, course: course).user
      second_id = insert_base_submission(assessment: assessment, creator: newer_creator, session_id: 'second')
      described_class.new.up
      @migration_reapplied = true

      expect([detail_count(first_id), detail_count(second_id)]).to eq([1, 1])
    end

    it 'backfills every base row even when they span multiple batches' do
      stub_const('CreateCourseAssessmentSubmissionDetails::BACKFILL_BATCH_SIZE', 2)
      ids = Array.new(3) do |i|
        insert_base_submission(assessment: assessment,
                               creator: create(:course_student, course: course).user, session_id: "batch-#{i}")
      end

      described_class.new.up
      @migration_reapplied = true

      expect(ids.map { |id| detail_count(id) }).to eq([1, 1, 1])
    end
  end
end
