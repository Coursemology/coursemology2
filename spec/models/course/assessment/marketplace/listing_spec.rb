  ...
  with_tenant(:instance) do
    ...
    describe 'versioning associations (additive; all nullable)' do
      ...
      it 'optionally references a source course, fallback maintainer, and provenance' do
        ...
        started_at = Time.zone.local(2024, 8, 12)
        ended_at = Time.zone.local(2024, 12, 6)
        listing.update!(source_course: course, source_course_name: 'Intro to AI',
                        source_course_code: 'CS2109S',
                        source_started_at: started_at, source_ended_at: ended_at,
    ...
    describe '#marketplace_hosted?' do
      ...
      # dedicated preview instance, so every admin request asks this question from a DIFFERENT tenant —
      # and a tenant-scoped `Course` lookup returns nil rather than raising, which would make this
      # answer `false` for precisely the listings it is meant to identify. The examples above cannot
      # catch it: their container sits in the caller's own instance.
