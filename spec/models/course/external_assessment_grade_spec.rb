...
RSpec.describe Course::ExternalAssessmentGrade, type: :model do
  ...
  with_tenant(:instance) do
    describe 'validations' do
      ...
      it 'allows a negative grade (penalties; floor applied via floor_at_zero, not validation)' do
        subject.grade = -5
        expect(subject).to be_valid
      end
      ...
      it 'allows the same course_user under a different external_assessment' do
        existing = create(:course_external_assessment_grade)
        other = build(:course_external_assessment_grade, course_user: existing.course_user)
        expect(other).to be_valid
      end

      it 'allows the same external_assessment for a different course_user' do
        existing = create(:course_external_assessment_grade)
        other = build(:course_external_assessment_grade,
                      external_assessment: existing.external_assessment)
        expect(other).to be_valid
      end
