  ...
  with_tenant(:instance) do
    ...
    describe 'dependent destroy' do
      it 'destroys the gradebook contribution and grades when destroyed' do
        ...
        expect do
          external.destroy
        end.to change(Course::Gradebook::ExternalContribution, :count).by(-1).
    ...
    describe '.create_for_course!' do
      ...
      it 'rolls back the external when contribution creation fails' do
        allow(Course::Gradebook::ExternalContribution).to receive(:create!).
          and_raise(ActiveRecord::RecordInvalid.new(Course::Gradebook::ExternalContribution.new))
        expect do
          expect do
            described_class.create_for_course!(course: course, title: 'Final', maximum_grade: 80.0)
          end.to raise_error(ActiveRecord::RecordInvalid)
