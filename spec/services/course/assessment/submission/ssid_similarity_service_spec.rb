...
RSpec.describe Course::Assessment::Submission::SsidSimilarityService do
  ...
  with_tenant(:instance) do
    ...
    before do
      ...
      allow(Course::Assessment::Submission::ZipDownloadService).to receive(:download_and_zip).
    ...
    describe '#run_similarity_check' do
      before do
        ...
        stubs.post(/folders\/.*\/similarity-checks/) do |env|
          expect(env[:url].to_s).to include("/folders/#{assessment.ssid_folder_id}/similarity-checks")
          [Ssid::ApiStubs::SEND_PLAGIARISM_CHECK_SUCCESS[:status],
           { 'Content-Type': 'application/json' },
           Ssid::ApiStubs::SEND_PLAGIARISM_CHECK_SUCCESS[:body]]
        end
        stubs.get(/folders\/.*\/similarity-checks/) do |env|
          expect(env[:url].to_s).to include("/folders/#{assessment.ssid_folder_id}/similarity-checks")
      ...
      it 'completes similarity check successfully and updates ssid ids' do
        response = subject.run_similarity_check
        expect(response['status']).to eq('successful')
        expect(course.reload.ssid_folder_id).to be_present
        expect(assessment.reload.ssid_folder_id).to be_present
      end
    end

    describe '#fetch_similarity_result' do
      before do
        stubs.get(/folders\/.*\/similarity-checks\/latest/) do |env|
          expect(env[:url].to_s).to include("/folders/#{assessment.ssid_folder_id}/similarity-checks/latest")
      ...
      it 'fetches similarity results successfully' do
        results = subject.fetch_similarity_result
        expect(results).to be_an(Array)
      end
    end

    describe '#download_submission_pair_result' do
      let(:submission_pair_id) { SecureRandom.uuid }
      before do
