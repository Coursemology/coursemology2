...
RSpec.describe Course::Assessment::Marketplace::PurgeService, type: :service do
  ...
  with_tenant(:instance) do
    ...
    # `orphan!` deletes the authoring assessment, which enqueues the automatic `RestoreAuthoringJob`
    # rebuild. Under the test env's default :background_thread adapter that job would run CONCURRENTLY
    # with the purge under test: it duplicates the snapshot — link-rowing it, so destroying the snapshot
    # trips course_assessment_links' foreign key — and un-orphans the listing, so `purgeable?` may be
    # false by the time the purge reads it. Enqueue without performing; the rebuild has its own spec.
