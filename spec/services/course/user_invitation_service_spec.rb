...
RSpec.describe Course::UserInvitationService, type: :service do
  ...
  with_tenant(:instance) do
    ...
    describe '#invite' do
      ...
      context 'when the CSV begins with a blank line' do
        it 'raises a header-validation error rather than a nil error' do
          csv = Tempfile.new(File.basename(__FILE__, '.*')).tap do |file|
          ...
          expect { subject.invite(csv) }.to raise_error(
            CSV::MalformedCSVError,
            /#{Regexp.escape(I18n.t('errors.course.user_invitations.missing_required_headers'))}/
          )
          csv.close!
        end
      end
