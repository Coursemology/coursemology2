...
RSpec.describe Course::Assessment::Marketplace::DuplicationJob, type: :job do
  ...
  with_tenant(:instance) do
    ...
    describe 'title collision' do
      ...
      #
      # 242, not 255, on purpose. The assessment's material folder is named after the title, and the
      # second copy's folder collides with the first's, so `Folder#assign_valid_name` appends " (0)"
      # — at 255 that overflows the FOLDER's own 255-char limit and the duplication dies before the
      # title logic is ever reached. 242 exceeds `255 - 14`, so the truncation branch is genuinely
      # exercised; the third import proves the truncated dated title is checked before persisting.
