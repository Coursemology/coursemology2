...
RSpec.describe 'Extension: Manual Eager Load', type: :model do
  class self::SourceItem < ActiveRecord::Base
    ...
    belongs_to :through_item, inverse_of: :source_items
  end
  class self::ReferencedItem < ActiveRecord::Base
    belongs_to :source_item, inverse_of: :referenced_items
    belongs_to :through_item, inverse_of: :referenced_items
  end
  class self::SingularItem < ActiveRecord::Base
    self.table_name = 'referenced_items'
    belongs_to :source_item, inverse_of: :referenced_items
  end
  class self::ThroughItem < ActiveRecord::Base
    has_many :source_items, inverse_of: :through_item
    has_many :referenced_items, through: :source_items, inverse_of: :through_item
  end

  temporary_table(:through_items) do |t|
    t.string :test
  end
  temporary_table(:source_items) do |t|
    t.string :test
    t.references :through_item, foreign_key: nil
  end
  temporary_table(:referenced_items) do |t|
    t.references :source_item, foreign_key: nil
  end
  with_temporary_table(:source_items) do
    with_temporary_table(:through_items) do
      with_temporary_table(:referenced_items) do
        let(:preloader) { ActiveRecord::Associations::Preloader::ManualPreloader.new }
        let(:through) { self.class::ThroughItem.create }
        let(:source) { self.class::SourceItem.create(through_item: through) }
        ...
        context 'when eager loading a has_many_through relation' do
          before do
            referenced1
            referenced2
            referenced3
            ...
            through.reload
            preloader.preload(through, :referenced_items, eager_load_items)
          end

          let(:eager_load_items) { [referenced1, referenced3] }
          it 'marks the association as loaded' do
            expect(through.referenced_items.loaded?).to be(true)
          end

          it 'preserves the order of the items' do
            expect(through.referenced_items.to_a).to eq(eager_load_items)
          end
        end
