...
class Course::Statistics::AssessmentsController < Course::Statistics::Controller
  ...
  def live_feedback_statistics
    ...
    load_submission_question_ids(@assessment.questions)
  ...
  def live_feedback_history
    ...
    load_submission_question_ids([@question])

    @messages = Course::Assessment::LiveFeedback::Message.
                joins(:thread).
                where(live_feedback_threads: { submission_question_id: @submission_question_ids }).
  ...
  def create_student_live_feedback_hash
    count_hash = Course::Assessment::LiveFeedback::Message.joins(:thread).
                 ...
                 where(live_feedback_threads: {
                   submission_question_id: @submission_question_ids,
    ...
    @student_live_feedback_hash = @all_students.to_h do |student|
      ...
      live_feedback_count = @ordered_questions.map { |question_id| count_hash[[student.user_id, question_id]] || 0 }
  ...
  def load_submission_question_ids(questions)
    @submission_question_ids = Course::Assessment::SubmissionQuestion.unscoped.
                               select(:id, :submission_id, :question_id).
                               where(submission_id: @submissions.pluck(:id),
                                     question_id: questions.pluck(:id)).pluck(:id)
  end
end
