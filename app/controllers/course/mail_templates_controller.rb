class Course::MailTemplatesController < Course::ModuleController
  ...
  helper_method :mail_action_options, :customized_mail_actions, :noncustomized_mail_actions
  ...
  def noncustomized_mail_actions(course)
    (Course::MailTemplate.actions.keys - course.mail_templates.map(&:action)).
