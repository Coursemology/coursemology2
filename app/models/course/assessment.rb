...
class Course::Assessment < ApplicationRecord
  ...
  # `linkable_tree_id` and rebuilds `linked_assessments`, which is right for course duplication but
  # wrong here: without this, the container snapshot, the origin assessment, and EVERY adopter's
  # copy end up mutual `linked_assessments` — exposing assessment ids across unrelated courses and
  # instances, and crashing any onward duplication of an adopted copy (the cross-course link set is
  # re-saved with an unresolvable course).
  #
  ...
  # publishable in its own right: the listing that would create has its source assessment frozen
  # inside the container, so nobody could ever edit it or cut a further version from it. Nothing on
  # the assessment row says this — a snapshot carries the origin's title verbatim — so the only
  # way to know is to read the version rows back (same reason `ListingVersion.labels_for_assessments`
  # exists).
  #
  ...
  # during the destroy — and it does so with `update_columns`, which fires no callbacks on the listing
  # itself, so the listing side offers nothing to hook. Only listings holding a published version are
  # remembered: the rebuild duplicates the latest snapshot, so one that never published has nothing to
  # rebuild from.
  ...
  # The marketplace goes on serving the snapshot either way — what orphaning actually costs is the
  # ability to publish a NEW version, and an admin has no way of learning that has happened short of
  # visiting the listings table. Doing it automatically means the manual "Rebuild source assessment"
  # action is only ever a retry after a failed job.
  #
