...
class Course::Assessment::Answer::ForumPostResponse < ApplicationRecord
  ...
  def compute_post(selected_post)
    ...
    selected_post.is_post_updated = post ? isLater(post.updated_at, selected_post.post_updated_at) : nil
  end
  ...
  def compute_parent(selected_post)
    ...
    selected_post.is_parent_updated = parent ? isLater(parent.updated_at, selected_post.parent_updated_at) : nil
  ...
  def isLater(target_time, ref_time)
