...
module Course::Assessment::Submission::WorkflowEventConcern
  ...
  # to the associated Submission if one exists. A preview Attempt (Phase 2) has no Submission, so
  # `submission` is `nil` and these are genuinely no-ops; that's the actual mechanism behind "hooks
  # that no-op by default", not polymorphism. The real bodies are defined on
  # Course::Assessment::Submission (see submission.rb).
  #
  # `assign_zero_experience_points` is shared by `finalise` and `resubmit_programming` in the old
  # code (workflow_event_concern.rb:24,95 pre-split) with no obvious single hook seam for the second
  # call site — `after_resubmit_programming_hook` is a third hook added for exactly that call,
  # flagged here as the residual judgment call the design spike left open (§5, §3.2).
