...
class Course::Assessment::Submission::SsidSimilarityService # rubocop:disable Metrics/ClassLength
  ...
  def run_similarity_check
    ...
    until poll_count >= MAX_POLL_RETRIES
      ...
      raise SsidError, { status: :ok, body: response_body } if response['status'] == 'failed'
  ...
  def fetch_similarity_result
    ...
    ssid_id_to_submission_hash = {}
    @assessment.submissions.where.not(ssid_submission_id: nil).each do |submission|
      ssid_id_to_submission_hash[submission.ssid_submission_id] = submission
    end
  ...
  def sync_ssid_submissions
    submission_id_to_ssid_id_hash = fetch_ssid_submissions.each_with_object({}) do |submission, hash|
      ssid_submission_id = submission['id']
      submission_id = submission['name'].split('_').first.to_i
      hash[submission_id] = ssid_submission_id
    end

    @ssid_id_to_submission_hash = {}
    @assessment.submissions.each do |submission|
      next unless submission_id_to_ssid_id_hash&.key?(submission.id)

      ssid_submission_id = submission_id_to_ssid_id_hash[submission.id]
      submission.update!(ssid_submission_id: ssid_submission_id)

      @ssid_id_to_submission_hash[ssid_submission_id] = submission
    end
  end
