...
#
# There is no copy step here, deliberately. The listing's current version snapshot ALREADY lives in
# the container course — `PublishService#snapshot_into_container` puts it there — and that same row is
# what a duplicate would hand an adopter. Previewing it is therefore faithful by construction: a
# preview cannot drift from the adopted copy, because they are the same assessment.
# Duplicating again per preview would reintroduce exactly that drift.
#
