...
class Course::Assessment::Question::KoditsuQuestionService
  ...
  def clean_from_comments(snippet)
    no_single_line_comments_snippet = snippet.gsub(/\/\/.*$/, '')
    no_multiple_line_comments_snippet = no_single_line_comments_snippet.gsub(/\/\*.*?\*\//m, '')

    no_multiple_backspaces = no_multiple_line_comments_snippet.
                             gsub(/\r/, '').
                             gsub(/\n\s{4}/, "\n").
                             gsub(/\n{2,}/, "\n")

    no_multiple_backspaces.strip
  end
  ...
  def truncate_google_test_framework_and_clean_comments(snippet)
    ...
    cleaned_snippet_from_comments = clean_from_comments(snippet)
