...
class Course::Assessment::Question::CodaveriProblemGenerationService
  ...
  def initialize(assessment, params, language, version)
    ...
    @payload = {
      ...
      llmConfig: {
        customPrompt: generate_payload_custom_prompt(custom_prompt),
  ...
  def generate_payload_custom_prompt(custom_prompt)
    (custom_prompt.length >= 500) ? "#{custom_prompt[0...495]}..." : custom_prompt
  end
