...
class Course::Gradebook::ExternalAssessmentImportService # rubocop:disable Metrics/ClassLength
  ...
  # Only the two modes the frontend can produce are accepted. Without this, any
  # unknown mode silently falls through to external-id lookup (see roster_lookup),
  # so a typo would resolve against the wrong column instead of failing loudly.
  def guard_identifier_mode!
    return if IDENTIFIER_MODES.include?(@identifier_mode)

    raise ImportError, { message: 'bad_identifier_mode', mode: @identifier_mode }
  end
