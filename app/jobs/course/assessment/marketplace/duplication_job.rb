...
class Course::Assessment::Marketplace::DuplicationJob < ApplicationJob
  ...
  # unrelated assessment of the same name is exactly as confusing, and previously landed silently.
  #
  ...
  # update banner uses, so the title and the banner can never disagree; `%d %b %Y` matches the
  # frontend's `formatLongDate`. It renders in the app's `Time.zone`, while the banner renders in the
  # browser's — near midnight the two can name adjacent days. Accepted: the stamp is a label to tell
  # two rows apart, not a timestamp anyone computes with.
  #
