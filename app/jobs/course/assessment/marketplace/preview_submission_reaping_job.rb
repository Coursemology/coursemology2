...
class Course::Assessment::Marketplace::PreviewSubmissionReapingJob < ApplicationJob
  ...
  # submission may linger up to a week past it. That slack is tolerable only because reaping is no
  # longer how a previewer starts over — `unique_assessment_id_and_creator_id` allows one submission
  # per (assessment, previewer), and the way to clear it is the banner's "Reset submission" button
  # (Course::Assessment::Marketplace::PreviewSubmissionsController), not waiting for the TTL. Tighten
  # the cron, not just this constant, if that ceiling ever has to be guaranteed.
