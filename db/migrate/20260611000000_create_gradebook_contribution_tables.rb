...
class CreateGradebookContributionTables < ActiveRecord::Migration[7.2]
  def change
    create_table :course_gradebook_contributions do |t|
      t.references :course, null: false, foreign_key: { to_table: :courses },
                            index: { name: 'fk__course_gradebook_contributions_course_id' }
      t.references :tab, null: true,
                         foreign_key: { to_table: :course_assessment_tabs, on_delete: :cascade },
                         index: { unique: true,
                                  name: 'index_course_gradebook_contributions_on_tab_id' }
      t.decimal :weight, precision: 5, scale: 2, null: false, default: 0
      t.integer :weight_mode, null: false, default: 0
      t.integer :keep_highest, null: false, default: 0

      t.references :creator, null: false, foreign_key: { to_table: :users },
                             index: { name: 'fk__course_gradebook_contributions_creator_id' }
      t.references :updater, null: false, foreign_key: { to_table: :users },
                             index: { name: 'fk__course_gradebook_contributions_updater_id' }
