...
#
# Deliberately ONE migration rather than a schema/data pair per slice. The data backfill calls
# application code (`PublishService.backfill_all!`), and application code always reflects this
# branch's FINAL schema — so it needs the rename and `courses.preview` to have already run. Split
# across separately-numbered migrations, a from-scratch `db:migrate` applied them in timestamp order
# and died on `PG::UndefinedColumn` (only `db:schema:load` masked it). Keeping schema-then-data
# inside one `up` makes that ordering unbreakable.
#
# A further schema change on this branch may be FOLDED IN here — but only into the schema group above,
# never appended below the backfills, which is what would break that ordering. The cost of folding is
# that any DB which already ran this migration will never pick the change up and has to be rebuilt
# from the template under a fresh tag; that is affordable only while this branch is unmerged. Once it
# lands, a further change needs a new migration.
class AddMarketplaceVersioningAndPreviewContainer < ActiveRecord::Migration[7.2] # rubocop:disable Metrics/ClassLength
  ...
  def add_listing_versioning_columns
    change_table :course_assessment_marketplace_listings, bulk: true do |t|
      ...
      # The origin course's own start/end dates, copied. Datetimes rather than a formatted
      # "Jul 2026 – Aug 2026" string: the client formats, and a string additionally sorts by month
      # NAME in the admin table. Denormalised for the same reason as `source_course_name` — the
      # course they were read from gets deleted (design V17).
      t.datetime :source_started_at
      t.datetime :source_ended_at
  ...
  def remove_listing_versioning_columns
    change_table :course_assessment_marketplace_listings, bulk: true do |t|
      t.remove :current_version_id, :source_course_id, :source_course_name, :source_course_code,
               :source_started_at, :source_ended_at, :fallback_maintainer_id
    end
  end
  ...
  #
  # An id, not a denormalised name string like `source_course_name`/`source_course_code` above: those
  ...
  # Design V17: listings published before this migration have no source dates. Fill them where the
  # source course still resolves. Idempotent — only touches rows with no `source_started_at`, which
  # is exactly the un-backfilled set: `courses.start_at`/`end_at` are both NOT NULL and the two
  # columns are always written together.
  def backfill_source_dates
    ActsAsTenant.without_tenant do
      Course::Assessment::Marketplace::Listing.
        where(source_started_at: nil).where.not(source_course_id: nil).
        includes(:source_course).find_each do |listing|
          # `update_columns` deliberately skips validations and callbacks: this is a pure data fill
          # and must not stamp `updated_at` or trip userstamp on rows whose creator context is gone.
          listing.update_columns(source_started_at: listing.source_course.start_at,
                                 source_ended_at: listing.source_course.end_at)
        end
    end
  end

  # Listings published before this migration read their instance off the surviving source course.
  # Rows already orphaned (`source_course_id IS NULL`) have nothing to read and stay NULL —
  # displayed as "—" forever. That's expected: the snapshot lives in the preview instance rather
  # than the origin, and a publisher can belong to several instances, so neither identifies where
  # the listing came from. Only listings orphaned after this migration keep it. Idempotent — only
  # touches NULL rows, same as `backfill_source_dates` above.
