class ApplySoftDeleteForCourseUserAndAssociatedModels1 < ActiveRecord::Migration[7.2]
  ...
  def down
    ...
    change_table :course_experience_points_records do |t|
      t.remove_index :deleted_at if index_exists?(:course_experience_points_records, :deleted_at)
      t.remove :deleted_at if column_exists?(:course_experience_points_records, :deleted_at)
    end

    change_table :course_learning_rate_records do |t|
      t.remove_index :deleted_at if index_exists?(:course_learning_rate_records, :deleted_at)
      t.remove :deleted_at if column_exists?(:course_learning_rate_records, :deleted_at)
    end

    change_table :course_user_achievements do |t|
      t.remove_index :deleted_at if index_exists?(:course_user_achievements, :deleted_at)
      t.remove :deleted_at if column_exists?(:course_user_achievements, :deleted_at)
    end

    change_table :course_user_email_unsubscriptions do |t|
      t.remove_index :deleted_at if index_exists?(:course_user_email_unsubscriptions, :deleted_at)
      t.remove :deleted_at if column_exists?(:course_user_email_unsubscriptions, :deleted_at)
    end

    change_table :course_group_users do |t|
      t.remove_index :deleted_at if index_exists?(:course_group_users, :deleted_at)
      t.remove :deleted_at if column_exists?(:course_group_users, :deleted_at)
    end

    change_table :course_personal_times do |t|
      t.remove_index :deleted_at if index_exists?(:course_personal_times, :deleted_at)
      t.remove :deleted_at if column_exists?(:course_personal_times, :deleted_at)
    end
