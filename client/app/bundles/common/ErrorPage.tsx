...
const NotFoundPage = (): JSX.Element => {
  ...
  // dropped rather than repointed, and it takes a second message rather than a conditional chunk: an
  // ICU tag cannot be omitted from a rendered message, and a `<home>` left without a handler renders
  // the tag literally. `CourselessContainer` forwards the root payload to this outlet, which is the
  // only place a courseless page can read the flag from.
  ...
  //
  // Read off `window.location` rather than through a route loader as `ForbiddenPage` does. That page
  // is only ever mounted at `/forbidden`; this one is also the catch-all's element, mounted in tests
  // under a plain `MemoryRouter`, where `useLoaderData` throws.
