...
const FETCH_FAILURE = 'Failed to load the marketplace access list.';
...
it('toasts when the fetch fails', async () => {
  mock.onGet(ACCESS_URL).reply(500);

  const page = renderSection();

  expect(await page.findByText(FETCH_FAILURE)).toBeVisible();
});

it('does not toast a failure from a fetch superseded by a rule-version change', async () => {
  let failFirstFetch: (reason?: unknown) => void = () => {};
  mock
    .onGet(ACCESS_URL)
    .replyOnce(
      () =>
  ...
  await act(async () => {
    failFirstFetch(new Error('superseded'));
  });

  expect(page.queryByText(FETCH_FAILURE)).not.toBeInTheDocument();
});
