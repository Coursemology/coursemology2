...
const activeUser = {
  ...
  instanceRole: null,
  allowedByRules: [
    { id: 10, ruleType: 'email_domain' as const, labelValue: 'nus.edu.sg' },
  ],
  systemAdmin: false,
  blocked: false,
  blockId: null,
...
it('renders the access list with annotations and a summary', async () => {
  ...
  expect(page.getAllByText('Email domain (nus.edu.sg)')).toHaveLength(2);
...
it('shows Everyone as the reason when the marketplace is open to everyone', async () => {
  ...
  expect(page.queryByText('Email domain (nus.edu.sg)')).not.toBeInTheDocument();
});
...
it('lists every rule that grants a user access', async () => {
    ...
    users: [
      {
        ...activeUser,
        allowedByRules: [
          { id: 10, ruleType: 'email_domain', labelValue: 'nus.edu.sg' },
  ...
  expect(await page.findByText('Email domain (nus.edu.sg)')).toBeVisible();
...
it('moves a block with no matching rule into the dormant list', async () => {
  ...
  expect(page.getByText('Dormant Dan')).toBeVisible();
...
it('treats a block as dormant only outside everyone-mode', async () => {
  ...
  const page = renderSection({ openToEveryone: true });
  await page.findByText('Dormant Dan');
...
it('clears a dormant block and drops the row', async () => {
  ...
  await waitFor(() =>
    expect(page.queryByText('Dormant Dan')).not.toBeInTheDocument(),
  );
  expect(page.queryByText(/^Dormant blocks/)).not.toBeInTheDocument();
});
...
it('labels a system admin in both reason columns', async () => {
  ...
  const page = renderSection();
  await page.findByText('Root Admin');

  expect(page.getAllByText('System admin')).toHaveLength(2);
...
it('filters system admins in and out via their own option', async () => {
  ...
  await toggleFilter(page, 'System admin');
  await waitFor(() =>
    expect(page.queryByText('Root Admin')).not.toBeInTheDocument(),
  );
  expect(page.getByText('Jane Tan')).toBeVisible();

  await toggleFilter(page, 'System admin');
  await waitFor(() => expect(page.getByText('Root Admin')).toBeVisible());
});
...
it('keeps a system admin listed when a rule box is unchecked', async () => {
  ...
  await toggleFilter(page, 'Email domain (nus.edu.sg)');

  await waitFor(() =>
    expect(page.queryByText('Jane Tan')).not.toBeInTheDocument(),
  );
  expect(page.getByText('Root Admin')).toBeVisible();
});
...
it('offers no system-admin option when nobody listed is one', async () => {
  ...
  expect(
    page.queryByRole('checkbox', { name: 'System admin' }),
  ).not.toBeInTheDocument();
});
...
// Anchor on the menu's role, not on its "Status" heading: the table has a Status column, so
// findByText('Status') matches both the column header and the filter panel.
...
const closeFilter = async (page: ReturnType<typeof render>): Promise<void> => {
  // MUI's Menu renders a modal backdrop over the table. Close it before asserting on, or typing
  // into, anything underneath — otherwise the assertions race the overlay rather than the filter.
...
it('hides the rule group when the marketplace is open to everyone', async () => {
  ...
  expect(
    page.queryByRole('checkbox', { name: 'Email domain (nus.edu.sg)' }),
  ).not.toBeInTheDocument();
});
