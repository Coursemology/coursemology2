...
it('previews the rule once when advancing to step 2', async () => {
  ...
  expect(
    await page.findByText(
      'Grants access to 5 of 12 eligible users · 7 already had access',
    ),
  ).toBeVisible();
...
it('names blocked matches apart from those who already had access', async () => {
  // The counts line used to derive its "already had access" number as matched - new, which swept
  // blocked people into it and claimed the rule granted them access. They are held back by their
  // own block, which the rule does not lift, so they are neither granted nor pre-existing.
  mock.onPost(PREVIEW_URL).reply(200, {
    matchedCount: 10,
    newCount: 6,
    blockedCount: 3,
  ...
  expect(
    await page.findByText(
      'Grants access to 6 of 10 eligible users · 1 already had access · 3 blocked individually',
    ),
  ).toBeVisible();
});

it('omits the already-had-access clause when every exclusion is a block', async () => {
  mock.onPost(PREVIEW_URL).reply(200, {
    matchedCount: 200,
    newCount: 197,
    blockedCount: 3,
  ...
  expect(
    await page.findByText(
      'Grants access to 197 of 200 eligible users · 3 blocked individually',
    ),
  ).toBeVisible();
});
...
it('resets to a clean step 1 when cancelled', async () => {
  ...
  await fillDomainAndAdvance(page);
  await page.findByText(
    'Grants access to 2 of 4 eligible users · 2 already had access',
  );
...
it('previews an instance rule, loading the instance list lazily and once', async () => {
  mock.onGet(INSTANCES_URL).reply(200, {
    instances: [
  ...
  await waitFor(() =>
    expect(
      mock.history.get.filter((r) => r.url === INSTANCES_URL),
    ).toHaveLength(1),
  );
  ...
  await page.findByText('Grants access to 3 eligible users');
  expect(JSON.parse(mock.history.post[0].data)).toEqual({
    allowlist_rule: { rule_type: 'instance', instance_id: 2 },
  });
  expect(mock.history.get.filter((r) => r.url === INSTANCES_URL)).toHaveLength(
    1,
  );
});

it('reports a failed instance-list fetch, and retries on the next selection', async () => {
  mock.onGet(INSTANCES_URL).replyOnce(500);
  mock.onGet(INSTANCES_URL).reply(200, {
    instances: [{ id: 2, name: 'Alpha', host: 'alpha.coursemology.org' }],
  });

  const { page } = renderForm();

  // By role, not by label: while the select's menu is closing it is still mounted, and its listbox
  // carries the same "Rule type" label as the field itself.
  const chooseRuleType = (name: string): void => {
    fireEvent.mouseDown(page.getByRole('combobox', { name: 'Rule type' }));
    fireEvent.click(page.getByRole('option', { name }));
  };

  await page.findByLabelText('Rule type');
  chooseRuleType('All eligible users in an instance');

  expect(await page.findByText('Failed to get instances')).toBeVisible();
  fireEvent.mouseDown(await page.findByRole('combobox', { name: 'Instance' }));
  expect(page.getByText('No options')).toBeVisible();

  // The failure must not be recorded as a load, or the admin would be stuck with an empty dropdown
  // for the rest of the dialog's life with no way to ask again.
  chooseRuleType('Specific eligible user');
  chooseRuleType('All eligible users in an instance');

  fireEvent.mouseDown(await page.findByRole('combobox', { name: 'Instance' }));
  expect(await page.findByRole('option', { name: 'Alpha' })).toBeVisible();
});
