...
const translations = defineMessages({
  ...
  // from and the one it is published from NOW, and after a rebuild those are different records. The
  // Actions link owns the live one under the plain name "source assessment"; this column owns the
  // historical one. Calling it ORIGINAL is what makes a struck-through cell literally true rather
  // than a claim that the listing is broken.
  colTitle: {
...
const MarketplaceListingsTable = ({
  ...
  const columns: ColumnTemplate<MarketplaceListingAdminData>[] = [
    ...
    },
    {
      ...
      //
      // The link merely navigates to the copy an admin edits; publishing happens on that page, which
      // is why the label says what it does rather than what it leads to. A listing with no authoring
      // copy renders no open action AT ALL rather than a disabled one — there is nothing to open, the
      // rebuild action beside it says what to do about that, and a disabled placeholder only added a
      // ragged extra line that misaligned those rows.
      //
      // List/unlist sits second because it is the action taken most often and the one delete depends
      // on: unlisting is the reversible step that has to precede a permanent deletion.
      //
      // The two maintenance actions have different scopes. Restore is orphaned-only — a listing that
      // still has a source assessment does not need one rebuilt — and additionally needs a version to
      // copy from.
      //
      // Delete is on EVERY row, disabled rather than absent where the listing is still published: the
      // set of deletable listings is unchanged, but a missing icon reads as "this table cannot delete"
      // and left the admin with nowhere to learn the rule. Disabled, it states the rule on hover
      // instead — unlist first, so the reversible step comes before the irreversible one. Adoption
      // count gates nothing at all: a deliberate deletion of an adopted listing must be allowed to
      // proceed, and its confirm dialog is where that fact is surfaced.
      cell: (listing) => (
