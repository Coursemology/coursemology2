...
const MarketplaceListingShow: FC = () => {
  ...
  // Matches the index table's treatment exactly, so the same fact does not arrive in two shapes:
  // struck through for the glance, suffixed for anyone who cannot see that, and a tooltip for the
  // part neither carries — that the listing itself still works.
  ...
  // Date AND time: this page puts versions side by side, and two cuts on the same day are exactly
  // what an admin comes here to tell apart.
  ...
  // when the instance was never recorded — the pre-column orphan case, which has no host either.
  ...
  // `//host/...`: Course is tenanted by instance, so a course id resolves only on its own host.
    ...
    <Page backTo="/admin/marketplace_listings" title={t(translations.header)}>
      <div className="mb-6 flex flex-col gap-2">
        <div className="flex items-center gap-3">
          {/* Plain text, and never struck through: this is the LISTING's identity, not the original
              assessment's name, so striking it would say the listing is dead when it is serving
              normally. The index table can strike its equivalent cell only because a column header
              spells out that the cell means the ORIGINAL; a heading has no such frame, so the
              deletion is stated on the provenance line below instead. Unlinked for the same reason —
              "Open source assessment" beside it is the entrance, and it names what it opens. */}
          ...
          {/* Beside the state chip, not instead of it: this page exists to answer provenance
              questions, and the Source course line below still names the ORIGIN course after a
              rebuild — so without this the page would be less truthful than the index table. */}
        ...
        <div className="flex flex-wrap gap-x-8 gap-y-1">
          ...
          {/* The deletion of the ORIGINAL, stated here rather than on the heading: the heading is the
              listing's identity, and marking it would say the listing is broken. Labelled, so it
              needs no strikethrough and no repeat of the title to be understood. */}
          ...
          {/* The assessment new versions are published from — after a rebuild a DIFFERENT record
              living in the marketplace's preview course, so it needs its own entrance rather than
              sharing the heading's link, which points at the original. */}
          ...
          <TableBody>
            {listing.versions.map((version) => (
              <TableRow key={version.publishedAt}>
                ...
                <TableCell>
                  {/* A new tab, not a navigation: the snapshot is a reference the admin returns
                      from, and leaving would discard the index table's search/sort/filter state. */}
                  {version.snapshotUrl ? (
                    <Link
                      external
