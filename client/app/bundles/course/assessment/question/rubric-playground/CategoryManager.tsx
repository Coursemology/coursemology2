...
const CategoryManager = (props: CategoryManagerProps): JSX.Element => {
  ...
  const isDirty = (currentCategories: RubricCategoryEntity[]): boolean => {
    ...
    return currentCategories.some((category, categoryIndex) => {
      ...
      if (
        ...
        category.criterions.length !== originalCategory.criterions.length
      ) {
        return true;
      }

      return category.criterions.some((criterion, criterionIndex) => {
        const originalGrade = originalCategory.criterions[criterionIndex];

        return (
          criterion.grade !== originalGrade.grade ||
          criterion.explanation !== originalGrade.explanation ||
          (criterion.toBeDeleted ?? false) !==
