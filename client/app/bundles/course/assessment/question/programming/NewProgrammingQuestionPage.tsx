...
const NewProgrammingQuestionPage = (): JSX.Element => {
  ...
  const loadPrefilledData = (
    ...
    if (prefilledData) {
      ...
      if (data.testUi) {
        data.testUi.metadata = Object.assign(
          data.testUi.metadata,
          prefilledMetadata,
        );
      } else {
        data.testUi = {
          metadata: prefilledMetadata,
          mode: data.languages.find(
            (lang) => lang.id === prefilledData.question.languageId,
          )!.editorMode,
        };
      }
    }
