...
const ProgrammingFiles = ({
  ...
  const isLiveFeedbackChatOpen =
    liveFeedbackChatForQuestion?.isLiveFeedbackChatOpen;
  ...
  return controlledProgrammingFields.map((field, index) => {
    ...
    return (
      <div
        key={keyString}
        className="flex w-full relative gap-3 mb-1 max-h-[100%]"
        id={keyString}
      >
        <Box className={`${isLiveFeedbackChatOpen ? 'w-1/2' : 'w-full'}`}>
          <ProgrammingFile
            key={field.id}
            answerId={answerId}
            editorRef={editorRef}
