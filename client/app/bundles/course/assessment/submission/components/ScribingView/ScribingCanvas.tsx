    ...
    const onMouseMoveCanvas = withCanvas(
      (
        ...
        const tryMove = (left: number, top: number): void => {
          ...
          finalLeft = Math.max(
            finalLeft,
            (canvas.getZoom() - 1) * canvas.getWidth() * -1,
          );
          let finalTop = Math.min(top, 0);
          finalTop = Math.max(
            finalTop,
            (canvas.getZoom() - 1) * canvas.getHeight() * -1,
          );
    ...
    useEffect(() => {
      ...
      const observer = new ResizeObserver(() => {
        ...
        rafId = requestAnimationFrame(() => {
          ...
          const ratio = container.getBoundingClientRect().width / canvasWidth;
          fabricWrapper.style.setProperty('zoom', String(Math.min(ratio, 1)));
        });
      });
