...
export const runAssessmentSimilarity = (
  assessmentId: number,
): Promise<void> => {
  return CourseAPI.similarity
    .runAssessmentSimilarity(assessmentId)
    .then((response) => {
      dispatch(
        actions.runSimilarityCheckSuccess({
          workflowState: ASSESSMENT_SIMILARITY_WORKFLOW_STATE.running,
          updatedAt: new Date(),
        }),
      );
      if (response.data.jobUrl) {
        startPollingSimilarityJob(response.data.jobUrl, assessmentId);
      }
    });
};
