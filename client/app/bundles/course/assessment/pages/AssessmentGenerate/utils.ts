...
const stringifyTestCases = <T extends MetadataTestCase | JavaMetadataTestCase>(
  ...
  testCases.forEach((testCase, index) => {
    ...
    if (includeInlineCode) {
      (testCaseDict[index + 1] as JavaMetadataTestCase).inlineCode = (testCase as JavaMetadataTestCase).inlineCode;
    }
  });
...
export const buildGenerateRequestPayload = (
  ...
  if (publicTestCases?.length > 0) {
    data.append(
      'public_test_cases',
      stringifyTestCases(publicTestCases, includeInlineCode),
    );
  }
  ...
  if (privateTestCases?.length > 0) {
    data.append(
      'private_test_cases',
      stringifyTestCases(privateTestCases, includeInlineCode),
    );
  }
  ...
  if (evaluationTestCases?.length > 0) {
    data.append(
      ...
      stringifyTestCases(evaluationTestCases, includeInlineCode),
    );
  }
