...
// its own `existing` prop (which carries the real weight). They are NOT on
...
export type UploadBlock =
  ...
  | { kind: 'identifierMissing'; expected: string }
  | { kind: 'noGradeColumns' }
  | { kind: 'noDataRows' }
...
// The header of the identifier column for the given mode, matched by name
// (case-insensitively) wherever it appears in the row — the identifier is not
// required to be first. Returns undefined when no column matches. Single source
// of truth shared by detectUploadBlock and useImportMapping.
export const findIdentifierColumn = (
  headers: string[],
  mode: IdentifierMode,
): string | undefined =>
  headers.find((h) => key(h) === key(canonicalIdentifierHeader(mode)));
...
// Identifier-column values with no matching student in the course roster,
// de-duplicated in first-seen order. Blank identifier cells are ignored
// (trailing/empty rows, e.g. an exported report's "Max Marks" row), not
// reported as unknown.
export const unknownIdentifiers = (
  ...
  // De-dup by the normalized key (not the raw string) so the same identifier in
  // different casing — e.g. two spellings of one email — is reported only once,
  // matching how `known` membership is tested below.
  const seen = new Set<string>();
  const unknown: string[] = [];
  rows.forEach((row) => {
    const raw = (row[identifierColumn] ?? '').trim();
    if (raw === '') return;
    const norm = normIdentifier(raw, identifierMode);
    if (seen.has(norm)) return;
    seen.add(norm);
    if (!known.has(norm)) unknown.push(raw);
  });
  return unknown;
};
...
export const classifyColumns = (
  ...
  return configs.map((c) => {
    ...
    return { ...c, status, error, badCells, hasNumericData };
  });
};
...
export const detectUploadBlock = (
  ...
  const identifierColumn = findIdentifierColumn(headers, identifierMode);
  if (identifierColumn === undefined) {
    return { kind: 'identifierMissing', expected };
  }
