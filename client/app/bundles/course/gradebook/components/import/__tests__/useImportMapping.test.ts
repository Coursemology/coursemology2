...
it('resolves the identifier column by name and lists the rest as columns', async () => {
...
it('resolves the identifier column by name even when it is not first', async () => {
  const { result } = await renderHook(() => useImportMapping(), {
    preloadedState,
  });
  act(() => {
    result.current.parseFile(
      'Name,Midterm,External ID,Final\nAlice,80,S1,90\n',
    );
  });

  expect(result.current.identifierColumn).toBe('External ID');
  // The identifier is excluded from the mappable columns wherever it sits.
  expect(result.current.columns.map((c) => c.header)).toEqual([
    'Name',
    'Midterm',
    'Final',
...
it('reports a structural upload block when no column matches the identifier', async () => {
  const { result } = await renderHook(() => useImportMapping(), {
    preloadedState,
  });
  act(() => {
