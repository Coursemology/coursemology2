...
it('shows a create row with an editable title and max-marks default', () => {
  renderTable([col({ header: 'Quiz 1', action: 'create', newTitle: 'Quiz 1' })]);
...
it('shows the placeholder for an unmapped existing row without any error text', () => {
  renderTable([col({ header: 'Final', action: 'existing', existingTarget: '', status: 'incomplete' })]);
...
it('shows the existing component max (looked up from the existing prop) as read-only', () => {
  ...
  renderTable([col({ header: 'Final', action: 'existing', existingTarget: 'Final' })]);
  const input = screen.getByDisplayValue('80') as HTMLInputElement;
  expect(input).toBeDisabled();
});

it('renders the Weightage column only when weighted view is on', () => {
  const columns = [col({ header: 'Quiz 1', action: 'create' })];
  const { rerender } = renderTable(columns);
  expect(screen.queryByText(/weightage/i)).toBeNull();
...
it('shows an inline error for a title collision', () => {
  renderTable([
    col({ header: 'Score', action: 'create', newTitle: 'Final', status: 'error', error: 'titleCollision' }),
  ]);
  expect(screen.getByText(/already used by an existing component/i)).toBeInTheDocument();
});
...
it('hides an existing option already taken by another row but keeps a row’s own pick', async () => {
  renderTable([
    col({ header: 'Quiz 1', action: 'existing', existingTarget: 'Final' }),
    col({ header: 'Quiz 2', action: 'existing', existingTarget: '', status: 'incomplete' }),
  ]);
  ...
  expect(screen.getByRole('option', { name: 'Other' })).toBeInTheDocument(); // free
  expect(screen.queryByRole('option', { name: 'Final' })).toBeNull(); // taken by Quiz 1
});

it('shows a quiet footer hint when a column still needs input', () => {
  renderTable([
    col({ header: 'Final', action: 'existing', existingTarget: '', status: 'incomplete' }),
  ]);
