...
export interface UseImportMapping {
  ...
  identifierColumn: string; // resolved by header name for the mode ('' if none)
...
const useImportMapping = (): UseImportMapping => {
  ...
  // The identifier column is located by header name (case-insensitively) for
  // the current mode, wherever it sits in the row — not by position. It stays
  // reactive to the mode toggle since the file isn't re-parsed on toggle.
  // detectUploadBlock blocks the upload when no column matches.
  const identifierColumn = useMemo(
    () => findIdentifierColumn(headers, identifierMode) ?? '',
    [headers, identifierMode],
  );
  ...
  const parseFile = useCallback(
    (data: string): void => {
      ...
      // Build a config for every column; the identifier column is excluded
      // reactively when deriving `columns` (below), since which column is the
      // identifier depends on the mode toggle and can change after parsing.
      const newConfigs: ColumnConfig[] = fields.map((header) => {
  ...
  const columns = useMemo(
    () =>
      classifyColumns(
        configs.filter((c) => c.header !== identifierColumn),
        rows,
        existing,
      ),
    [configs, rows, existing, identifierColumn],
  );
