...
const ExternalGradeCell = ({
  ...
  // Sync from the incoming prop only when it genuinely changes (a server
  // correction or a background refresh), not on every editing toggle. Resyncing
  // on the commit-time editing→false transition would clobber the optimistic
  // value we just set with the still-stale prop, before the store round-trip
  // has caught up.
  useLayoutEffect(() => {
