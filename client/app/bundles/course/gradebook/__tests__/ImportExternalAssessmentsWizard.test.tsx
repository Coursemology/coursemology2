...
describe('ImportExternalAssessmentsWizard', () => {
  ...
  it('drops a CSV, advances to Map, and shows the flat mapping table', async () => {
    ...
    // The identifier is a static label now, not a Select — it shows the column
    // resolved by header name and the active match mode alongside it.
    expect(
  ...
  it('tells the user which identifier column the CSV must contain, reacting to the toggle', async () => {
    renderWizard({}, [student({ id: 1, name: 'Alice', externalId: 'E1' })]);
    expect(
      screen.getByText(/must include a column named External ID/i),
    ).toBeInTheDocument();

    await userEvent.click(screen.getByRole('radio', { name: 'Email' }));
    expect(
      screen.getByText(/must include a column named Email/i),
    ).toBeInTheDocument();
  });
  ...
  // The identifier column is located by header name (case-insensitively),
  // wherever it sits in the row — not by position. detectUploadBlock only
  // blocks the upload when NO column matches the selected mode's canonical
  // header, so Next stays disabled on Upload with the block alert naming the
  // column the file is missing.
  it('blocks Next on the Upload step when no column matches the identifier header', async () => {
    renderWizard();
    // "Student ID" does not case-insensitively match the canonical "External
    // ID" header for external_id mode, and no other column does either.
    await userEvent.upload(
      screen.getByLabelText(/upload/i),
    ...
    expect(nextButton()).toBeDisabled();
    expect(
      screen.getByText(/needs a column named .*External ID/i),
    ).toBeInTheDocument();
  });

  it('accepts the identifier column in any position, not just first', async () => {
    (CourseAPI.gradebook.importPreview as jest.Mock).mockResolvedValue(
      okPreview(),
    );
    renderWizard();
    // Identifier ("External ID") is the SECOND column, after a grade column.
    // The upload must not be blocked; the identifier is resolved by name.
    await dropCsv(`${MIDTERM},${EXTERNAL_ID}\n41,${A001}\n`);
    expect(nextButton()).toBeEnabled();

    await userEvent.click(nextButton());
    // Map step shows the identifier (by name) as a static label and Midterm as
    // the one mappable grade column.
    expect(
      screen.getByText(`${EXTERNAL_ID} (${EXTERNAL_ID})`),
    ).toBeInTheDocument();
    expect(screen.getByText(MIDTERM)).toBeInTheDocument();
  ...
  it('sends the resolved identifier column to the backend when it is not first', async () => {
    (CourseAPI.gradebook.importPreview as jest.Mock).mockResolvedValue(
      okPreview(),
    );
    (CourseAPI.gradebook.importCommit as jest.Mock).mockResolvedValue(
    ...
    renderWizard();
    await advanceToPreview(`${MIDTERM},${EXTERNAL_ID}\n41,${A001}\n`);
    await screen.findByText(A001);
    await userEvent.click(
      screen.getByRole('button', { name: /confirm import/i }),
    );
  ...
  it('opens the conflict prompt and commits with keep/replace', async () => {
    ...
    expect(
      await screen.findByText(
        /1 row contains changes to existing grades\. After checking this preview, click Resolve Conflicts to review these conflicts before anything is imported\./i,
      ),
    ).toBeInTheDocument();
  ...
  it('shows preview subtitle with total row count when preview has more than 5 rows', async () => {
    ...
    expect(
      screen.getByText(
        /Previewing the first 5 of 6 rows. Check that these grades match your CSV before continuing./i,
      ),
    ).toBeInTheDocument();
  });
  ...
  it('shows all rows subtitle variant when totalRows is 5 or fewer', async () => {
    ...
    expect(
      screen.getByText(
        /Previewing all 5 rows. Check that these grades match your CSV before continuing./i,
      ),
    ).toBeInTheDocument();
  });
  ...
  it('hides the wizard while the conflict prompt is open', async () => {
    (CourseAPI.gradebook.importPreview as jest.Mock).mockResolvedValue(
      okPreview({
        sample: [{ identifier: A001, grades: { Midterm: 20 } }],
        conflictRows: [
    ...
    // The conflict prompt is the only dialog on screen; the wizard is hidden.
    await screen.findByRole('dialog', { name: /resolve grade conflicts/i });
    expect(
      screen.queryByRole('dialog', { name: /import external assessments/i }),
    ).not.toBeInTheDocument();
  });

  it('re-shows the wizard when Go Back is clicked on the conflict prompt', async () => {
    (CourseAPI.gradebook.importPreview as jest.Mock).mockResolvedValue(
      okPreview({
        sample: [{ identifier: A001, grades: { Midterm: 20 } }],
        conflictRows: [
    ...
    await userEvent.click(
      within(prompt).getByRole('button', { name: /go back/i }),
    );

    // Wizard is back on the Preview step; the prompt is gone.
    expect(
