...
describe('<ManageUsersTable />', () => {
  ...
  describe('sort by External ID', () => {
    it(
      'clusters blank External IDs together when the column is sorted',
      async () => {
        const user = userEvent.setup();
        const users: CourseUserMiniEntity[] = [
        ...
        render(<ManageUsersTable csvDownloadFilename="users.csv" users={users} />);
        await waitForElementToBeRemoved(() => screen.queryByRole('progressbar'));

        const extIdHeader = screen.getByRole('columnheader', { name: /external id/i });
        await user.click(within(extIdHeader).getByRole('button'));

        await waitFor(() => {
          const rows = screen.getAllByRole('row').slice(1); // drop the header row
          const order = rows.map((r) => within(r).getByText(/Lim|Tan|Ng/).textContent);
