...
const EXT_ID_UPDATES_1 = 'External ID updates (1)';
...
describe('InvitationResultDialog', () => {
  ...
  it('renders External ID updates before New Invitations in DOM', async () => {
    renderDialog({
      newInvitations: [baseInvitation],
    ...
    const headings = screen
      .getAllByRole('heading')
      .map((h) => h.textContent ?? '');
    const updIdx = headings.findIndex((h) => h.includes('External ID updates'));
    const newInvIdx = headings.findIndex((h) => h.includes('New Invitations'));
  ...
  it('shows a single combined count and both sub-tables when both arrays are non-empty', async () => {
    ...
    await waitForElementToBeRemoved(() => screen.queryByRole('progressbar'));
    expect(screen.getByText('External ID updates (2)')).toBeInTheDocument();
    expect(screen.getByText('Invitations (1)')).toBeInTheDocument();
    expect(screen.getByText('Enrolled members (1)')).toBeInTheDocument();
  ...
  describe('pending external-ID toggle', () => {
    ...
    it('shows the flagged status line and Apply button when diffs exist', async () => {
      ...
      expect(
        screen.getByRole('button', { name: /Apply file values \(1\)/ }),
      ).toBeInTheDocument();
      expect(screen.getByText(EXT_ID_UPDATES_1)).toBeInTheDocument();
      expect(screen.getByText('Enrolled members (1)')).toBeInTheDocument();
    });

    it('applies, swaps to Undo, shows updated status line', async () => {
    ...
    it('undo reverts to previous values and restores Apply button', async () => {
      const user = userEvent.setup();
      renderDialog(pendingResult);
      await waitForElementToBeRemoved(() => screen.queryByRole('progressbar'));
      ...
      expect(operations.updateExternalIds).toHaveBeenLastCalledWith(
        expect.arrayContaining([
          expect.objectContaining({
            type: 'course_user',
            id: 1,
    ...
    it('shows no Apply affordance when there are no diffs', async () => {
      renderDialog({ newInvitations: [baseInvitation] });
      await waitForElementToBeRemoved(() => screen.queryByRole('progressbar'));
      expect(
        screen.queryByRole('button', { name: /Apply file values/ }),
      ).not.toBeInTheDocument();
      expect(
    ...
    it('on apply failure shows error toast and leaves button at Apply', async () => {
      ...
      // Button stays at Apply (not swapped to Undo)
      expect(
        screen.getByRole('button', { name: /Apply file values \(1\)/ }),
      ).toBeInTheDocument();
      expect(
    ...
    it('renders no dedicated diff section when there are no diffs', async () => {
      renderDialog({ newInvitations: [baseInvitation] });
      await waitForElementToBeRemoved(() => screen.queryByRole('progressbar'));
      expect(screen.queryByText(/External ID updates/)).not.toBeInTheDocument();
    });
  });
});
