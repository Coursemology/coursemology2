...
import MuiColumnPickerPrompt from '../MuiTableAdapter/MuiColumnPickerPrompt';

const TITLE = 'Select columns';
...
const makeRender = (ids: readonly string[]): jest.Mock =>
  jest.fn((ctx: ColumnPickerRenderContext) => (
    <>
      {ids.map((id) => (
        <label key={id}>
          <input
...
const setup = (
  overrides: Partial<React.ComponentProps<typeof MuiColumnPickerPrompt>> = {},
): ReturnType<typeof render> & {
  commitColumnVisibility: jest.Mock;
  props: React.ComponentProps<typeof MuiColumnPickerPrompt>;
} => {
  ...
  return {
    ...render(wrap(<MuiColumnPickerPrompt {...props} />)),
    commitColumnVisibility,
    props,
  };
};

describe('MuiColumnPickerPrompt', () => {
  ...
  describe('locked column behavior', () => {
    const makeGroupRender = (ids: readonly string[]): jest.Mock =>
      jest.fn(
        (ctx: ColumnPickerRenderContext): JSX.Element => (
          <>
            <button
      ...
      render(
        wrap(
          <MuiColumnPickerPrompt
            columnPicker={{
              render: makeGroupRender(['name', 'email']),
              dialogTitle: TITLE,
            }}
