...
const col = (
  ...
  ({ id, title: id, cell: () => null, ...overrides }) as ColumnTemplate<D>;
...
describe('buildHeaderRows', () => {
  ...
  it('dev warning: duplicate column ids warns in development', () => {
    const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
    const orig = process.env.NODE_ENV;
    process.env.NODE_ENV = 'development';
    try {
      buildHeaderRows([col('a'), col('b'), col('a')], buildLeaf);
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining('duplicate column id "a"'),
      );
    } finally {
  ...
  it('dev no-warning: different tabs under the same category produce separate leaf columns', () => {
    const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
    const orig = process.env.NODE_ENV;
    process.env.NODE_ENV = 'development';
    try {
      const rows = buildHeaderRows(
        [
          col('a', {
            groupPath: [
              { id: 'cat1', title: 'Mission' },
              { id: 'tab1', title: 'Exercises' },
            ],
          }),
          col('b', {
            groupPath: [
      ...
      // category row: one spanning cell for Mission
      expect(rows[0].cells).toHaveLength(1);
      expect(rows[0].cells[0].colSpan).toBe(3);
      // tab row: three separate cells, one per tab
      expect(rows[1].cells).toHaveLength(3);
      rows[1].cells.forEach((cell) => expect(cell.colSpan).toBe(1));
      // leaf row: three columns
  ...
  it('dev no-warning: same tab id under different categories is valid and does not warn', () => {
    const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
    const orig = process.env.NODE_ENV;
    process.env.NODE_ENV = 'development';
    try {
      buildHeaderRows(
        [
          ...
          col('c', {
            groupPath: [
              { id: 'cat2', title: 'Exams' },
              { id: 'exercises', title: 'Exercises' }, // same id, different parent — valid
            ],
          }),
        ],
        buildLeaf,
