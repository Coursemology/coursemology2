 ...
 *
 * Keyboard: arrows move and select; roving tabindex keeps it a single tab stop.
 */
const SegmentedSwitch = <T extends string | number>(
  ...
  // Slide the thumb under the active segment. Segments are content-width, so
  // both offset and width are measured. Re-measure on selection change and on
  // any resize (font load, container reflow) — a one-shot measure would leave
  // the thumb stranded after either.
  useLayoutEffect(() => {
    const measure = (): void => {
      const el = optionRefs.current[activeIndex];
      const container = containerRef.current;
      if (!el || !container) return;
      // `offsetLeft` is measured from the container's border box, but the thumb
      // (`left: 0`) is anchored to its padding box — just inside the border.
      // Subtract the border width (`clientLeft`) so the two share a coordinate
      // system; without it the active-left thumb loses its inset and sits flush
      // against the border.
      setThumb({
  ...
  return (
    ...
    >
      {/* The sliding thumb. Hidden until measured (width 0) to avoid a flash
          from the top-left corner on first paint. */}
      <Box
        aria-hidden
        sx={(theme) => ({
          position: 'absolute',
        ...
        const segment = (
          <ButtonBase
            ...
            sx={(theme) => ({
              ...
              px: PADDING_X,
              // Height comes from the track's `minHeight` + `align-items:
              // stretch`; the segment fills it and centers its label, so no
